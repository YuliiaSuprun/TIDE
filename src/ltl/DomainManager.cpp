...
const map<int, string>& DomainManager::get_bdd_to_ap_map() const {
    return bdd_to_ap_;
}
...
string DomainManager::get_ap_name(const int var_num) {
    auto it = bdd_to_ap_.find(var_num);
    if (it != bdd_to_ap_.end()) {
        return it->second;
    } else {
        cerr << "Error: Variable number " << var_num << " not found in the bdd_to_ap_ map." << endl;
        return ""; 
    }
}
