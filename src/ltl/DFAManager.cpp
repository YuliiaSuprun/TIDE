...
void DFAManager::construct_dfa(const LTLFormula& formula) {
    ...
    trans.set_pref(spot::postprocessor::SBAcc | spot::postprocessor::Deterministic);
    ...
    // dfa_ = spot::scc_filter_states(spot::to_finite(translated_dfa));
...
void DFAManager::save_dfa(const std::string& filename, const std::string& subdirname) {
    ...
    // Render the DFA .dot file to a PNG image using Graphviz
    GVC_t* gvc = gvContext();
    if (!gvc) {
        cerr << "Failed to create Graphviz context." << endl;
        return;
    }
    FILE* file = fopen(dot_filename.c_str(), "r");
    if (!file) {
        cerr << "Failed to open " << dot_filename << " for reading." << endl;
        gvFreeContext(gvc);  // Clean up context
        return;
    }

    Agraph_t* graph = agread(file, 0);
    if (!graph) {
        cerr << "Failed to read dot file: " << dot_filename << endl;
        fclose(file);
        gvFreeContext(gvc);  // Clean up context
        return;
    }
    if (gvLayout(gvc, graph, "dot") != 0) {
        cerr << "Graphviz layout failed." << endl;
        agclose(graph);
        fclose(file);
        gvFreeContext(gvc);  // Clean up context
        return;
    ...
    if (gvRenderFilename(gvc, graph, "png", png_filename.c_str()) != 0) {
        cerr << "Graphviz rendering failed." << endl;
...
shared_ptr<DFANode> DFAManager::generate_dfa_path() {
    size_t max_iteration_interval = 50;
    ...
    while (!nodePriorityQueue_.empty()) {
        ...
        if (is_accepting_state(current_dfa_state)) {
            ...
            if (currentPathCost < bestScore) {
                ...
                // Heuristic (can be proven that we got in a cycle)
                if (bestScore > CYCLE_COST) {
                    cout << "Terminating search..." << endl;
                    // Finish the search.
                    nodePriorityQueue_.clear();
                } else {
            ...
            continue;
        } else if (bestAcceptingNode && num_iterations > max_iteration_interval) {
            // Return the better path.
            // auto dfa_trace = bestAcceptingNode->getPathFromRoot();
            // cout << "Selected a DFA trace of size " << dfa_trace.size() - 1 << " and a cost of " << bestScore << endl;
            // print_dfa_path(dfa_trace);
        ...
        for (auto& edge: get_transitions(current_dfa_state)) {
            ...
            if (next_dfa_state != current_dfa_state && is_transition_feasible(edge.cond)) {
                ...
                if (currentNode->repeated_edge(next_dfa_state)) {
                    edge_cost += CYCLE_COST;
                }
...
int DFAManager::count_differing_aps(const bdd& bdd1, const bdd& bdd2) {
    ...
    for (const auto& [formula, var_num] : bdd_dict_->var_map) {
