...
int main(int argc, char** argv) {
    if (argc < 3) {
        cerr << "Usage: " << argv[0] << " <Directory Path> <Number of Runs> [--method <'exp', 'poly', 'plan4past', 'fond4ltlf'>] [--search <'lama-full', 'lama-first'>] [--goal <Goal JSON File>] [--timeout <in ms>]" << endl;
        exit(EXIT_FAILURE);
    }
