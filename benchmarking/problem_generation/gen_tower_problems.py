...
def generate_pddl_problem(num_blocks, directory):
    ...
    goal_next = " ".join(f"(on b{i} b{i+1})" for i in range(num_blocks - 1, 0, -1))
...
"""
    ...
    file_path = os.path.join(directory, f"t{num_blocks}.pddl")
...
def main():
    ...
    start, end = 3, 50
    output_directory = "benchmarking/data/deterministic/LTLf/S24/blocksworld/"
